...
@task
@needs(
    'simavr_make_all',  # make generates some header file, scons needs them
)
def scons():
...
@task
def simavr_make_all():
    sh('make all', cwd='pysimavr/swig/simavr')


@task
def simavr_make_clean():
    sh('make clean', cwd='pysimavr/swig/simavr')
